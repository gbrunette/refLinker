...
////////////////////////////////////////////////////////////////////////////////////////////////////////
void read_het_coverage_greg( std::string coverageFile, variant_graph& vgraph, vcf_vector vvec, std::vector<int>& pop_positions ) { 
    ...
    while (getline(inputFile,line)) {
        ...
        pop_it = find(pop_positions.begin(),pop_positions.end(), pos);
        int rnum=0; int vnum=0;
        char r = ref[0];
        char v = var[0];
        switch(r) {
                case 'A'  :
        ...
        if (pop_it != pop_positions.end() && anum+tnum+cnum+gnum-rnum-vnum < 5 && (rnum + vnum) > 0 && (rnum + vnum) < 50 ) {
            variant_node v_node1,v_node2;
            vgraph[variant_id] = v_node1;  
            vgraph[reference_id] = v_node2;
            vgraph[variant_id].reset_values(pos,true,var,ref,anum,tnum,cnum,gnum,dnum,inum);
            ...
            vgraph[variant_id].paired = true;
            vgraph[reference_id].paired = true;
            //cout << "tot_cov " << anum+tnum+cnum+gnum << endl;
        }
        else { 
                if ( pop_it != pop_positions.end() ) {
                        //cout << pos << "\t" << tot_cov << "\t" << *pop_it << endl; 
                        pop_positions.erase(pop_it); 
                        //std::vector<int>::iterator it2 = find(pop_positions.begin(),pop_positions.end(), pos);
                        //cout << *it2 << endl; 
                } 
...
void read_scaffold( std::string scaffile, coord_dictionary& pdict ) {
        ...
        while (getline(inputfile,line)) {
                ...
                std::string ref_name,alt_name,arm,chrom,block_str,tenx_str;
                double tenx_flipE,tenx_blockE;
                ss >> arm >> pos >> ref_name >> alt_name >> hap >> block_str >> tenx_flipE >> tenx_blockE;
                pdict.ref_handle.push_back(ref_name);
                pdict.alt_handle.push_back(alt_name);
                pdict.double_positions.push_back(pos);
                pdict.all_positions.push_back(pos);
